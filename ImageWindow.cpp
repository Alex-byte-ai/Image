    ...
    : outputData( image_ ), handleMsg( handleMsg_ ), image( image_ ), data( initData )
{
    ...
    auto windowProc = []( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam ) -> LRESULT
    {
        ...
        static ImageData canvas;

        auto resetInput = [d]()
        {
            if( d )
            {
                d->window->inputData.escape.reset();
                d->window->inputData.shift.reset();
                d->window->inputData.space.reset();
                d->window->inputData.leftMouse.reset();
                d->window->inputData.rightMouse.reset();
                d->window->inputData.middleMouse.reset();
                d->window->inputData.mouseX.reset();
                d->window->inputData.mouseY.reset();
                d->window->inputData.keys.reset();
            }
        };
